...
async def test_legacy_sensors(
    ...
    """Test the sensors with the legacy API."""
    with patch(
        ...
        state = hass.states.get("sensor.meteolux_temp_max")
        assert state.state == "16.0"


async def test_json_sensors(
    hass: HomeAssistant, json_data: dict, config_entry: ConfigEntry
) -> None:
    """Test the sensors with the JSON API."""
    config_entry.data = {
    ...
    with patch(
        ...
        state = hass.states.get("sensor.meteolux_sunshine")
        assert state.state == "4"

        state = hass.states.get("sensor.meteolux_uv_index")
